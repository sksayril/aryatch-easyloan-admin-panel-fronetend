...
export const adsSettingsApi = {
  get: async () => {
    const response = await fetch(`${API_BASE_URL}/admin/ads-settings`, {
      headers: getAuthHeaders(),
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.message || 'Failed to fetch ads settings');
    return data;
  },

  update: async (updates: AdsSettingsPayload) => {
    const response = await fetch(`${API_BASE_URL}/admin/ads-settings`, {
      method: 'PUT',
      headers: getAuthHeaders(),
      body: JSON.stringify(updates),
